...
class Pws_hiburan_m extends CI_Model {

    public function get($id=null)
    {
        $this->db->select('pws_hiburan.*, wp.*, tipe_wp.*');
        $this->db->from('pws_hiburan');
        $this->db->join('wp', 'pws_hiburan.npwpd = wp.npwpd');
        $this->db->join('tipe_wp', 'pws_hiburan.tipe_id = tipe_wp.tipe_id');
        if($id != null){
    ...
    public function get_npwpd($id)
    {
        ...
        $this->db->join('wp', 'pws_hiburan.npwpd = wp.npwpd');
        $this->db->join('tipe_wp', 'pws_hiburan.tipe_id = tipe_wp.tipe_id');
        $this->db->where('pws_hiburan.npwpd', $id);
        $query = $this->db->get();
        return $query;
    }
    ...
    public function add($post)
    {
        ...
        $this->db->insert('pws_hiburan', $params);
    }
    ...
    public function verif($id)
    {
        ...
        $this->db->where('no_pws', $id);
        $this->db->update('pws_hiburan', $params);
    }
    ...
    {
        $array = array('jumlah >' => '0', 'pws_hiburan.npwpd =' => $id);
        $this->db->select('pws_hiburan.*, wp.*, tipe_wp.*');
        $this->db->from('pws_hiburan');
        $this->db->join('wp', 'pws_hiburan.npwpd = wp.npwpd');
        $this->db->join('tipe_wp', 'pws_hiburan.tipe_id = tipe_wp.tipe_id');
